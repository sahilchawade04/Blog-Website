...
const UpdateBlog = () => {
    ...
    const selectBlog = blog.find(blog => blog._id === Number(id)) || blog.find(blog => blog._id === id); // Handle both string/number types
    
    // Fallback if blog not found (cleaner than crashing)
    ...
    const updateBlogHandler = async () => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                // Update Redux state
                const updatedBlogs = blog.map(b => b._id === res.data.blog._id ? res.data.blog : b);
                dispatch(setBlog(updatedBlogs));
                
                // Optionally navigate to view or just stay nicely
                // navigate(`/blog/${id}`);
            }
        } catch (error) {
            console.log(error);
